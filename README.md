<p align="center">
  <img src="assets/banner.jpg" alt="Розенталь — корректор русского текста, страж нормы">
</p>

# Розенталь

> **Claude Code-скил (команда `/rozental`): корректор русского текста. Орфография, пунктуация, согласование, управление, типографика, единообразие. НЕ смысл и не голос — только формальные ошибки нормы. Два режима: разобрать со ссылками на § правил или вычитать на месте. Главный принцип — precision важнее recall: гиперкоррекция (исправление верного на неверное) для корректора хуже пропуска.**
...
Даёте текст — Claude проходит по нему как корректор: типографика (ёлочки, тире, неразрывные
пробелы, юникод-гигиена), орфография (-тся/-ться, н/нн, словарные опечатки), пунктуация
(обособления, парные знаки, прямая речь, слова-ловушки), согласование и управление,
единообразие. Возвращает таблицу флагов с цитатами, обязательной ссылкой на правило — и
(в режиме Правка) вычитанный текст. Спорное и редполитика молча не правятся.

```
/rozental <вставьте текст>
→ Режим: Правка. Найдено правок: 14 (механика: 6, орфо: 2, пункт: 4, согл: 1, ед-во: 1).
→ ВЫЧИТАННЫЙ ТЕКСТ: ...
→ На решение автора: 2 спорных места — ёфикация, тире vs двоеточие в БСП.
```

<p align="center">
  <img src="assets/hero.jpg" alt="Корректор с красным карандашом над текстом; на полях аккуратные пометки и ссылки на параграфы"><br>
  <sub><i>«Справочник по правописанию» — против тихих ошибок и тихих гиперкоррекций</i></sub>
</p>
...
1. **Типографика и юникод-гигиена.** Ёлочки и лапки, тире (длинное/короткое/дефис),
   неразрывные пробелы, многоточие, чистка омоглифов и zero-width — code-блоки, URL и
   markdown не трогаются.
2. **Орфография.** -тся/-ться (вопрос к глаголу); н/нн; слитно/раздельно/дефис; «не» с
   частями речи; приставки пре-/при-, з/с; чередующиеся корни; о/ё после шипящих; заглавные;
   словарные опечатки по closed-list.
3. **Пунктуация.** Запятые (однородные, ССП/СПП, обособления), тире и двоеточие в БСП,
   прямая речь, **слова-ловушки** (вряд ли, якобы, будто, ведь, как раз, всё-таки — НЕ
   выделять запятыми).
4. **Согласование и управление.** Числительные, собирательные («двое суток»), сказуемое при
   «большинство/ряд/часть», аббревиатуры, трудные пары управления (согласно + дательный;
   оплатить что / заплатить за что).
5. **Единообразие.** Собрать разнобой (кавычки, тире, написания имён, формат чисел/дат) и
   привести к одному канону. Спорное — в «На решение автора», не молча.
6. **Самопроверка (анти-гиперкоррекция).** Перечитать правки: каждая опирается на §?
   Не исправил ли верное? Не тронул ли авторский приём? Перепроверить, что неразрывные
   пробелы и ёлочки не съелись при переписывании текста — это любимая тихая потеря.
...
- **Типографика:** прямые `"кавычки"` → «ёлочки»; дефис между словами → тире (—); диапазон
  чисел → короткое тире (3–5, 1941–1945); неразрывный пробел после однобуквенных предлогов
  (в`·`лесу), число + единица (5`·`кг), № 5; многоточие `...` → `…`; двойные пробелы и
  пробел перед знаками — убрать.
- **Юникод-гигиена:** омоглифы (латинские буквы внутри кириллицы и наоборот), zero-width
  пробелы, BOM, mojibake — вычистить. Code-блоки, URL и markdown не тронуты.
...
- **Пунктуация:** обособление определений/обстоятельств/приложений/уточнений; парные знаки
  ...
  тире/двоеточие в БСП; прямая речь.
...
- **Единообразие:** кавычки, тире, написания имён/терминов, формат чисел/дат — один канон
  в пределах текста.
...
- **Единообразие** — зафиксировать стайл-лист (какие кавычки в этом проекте, как пишутся
  бренды, тире vs двоеточие в БСП).
...
формальные ошибки нормы (орфография, пунктуация, согласование, типографика) и не трогает
фразы, даже если они неуклюжие. Если фраза грамматически верна, корректор её не меняет.

**Чем отличается от редактора (Чуковского / Норы Галь)?** Редактор работает со смыслом и
формой: канцелярит, цепи родительных, неясный тезис, сломанная структура, длинные
предложения. Корректор работает с буквой: -тся/-ться, запятые, ёлочки. Это разные задачи,
обычно делаются последовательно — сначала редактор, потом корректор.
